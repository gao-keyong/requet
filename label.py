...
def labeling(tag_folder):
    ...
    Resolution_list = ['q144p', 'q240p', 'q360p',
                       'q480p', 'q720p', 'q1080p', 'q1440p', 'q2160p']

    for dataset in dataset_list:
        dataset_folder = tag_folder + '/' + dataset + '/'
    ...
    for dataset in dataset_list:
        ...
        for file in files:
            ...
            df = pd.read_csv(file)
            df_new = pd.DataFrame(
                columns=["EpochTime",  "BuffWarning", "status", "Resolution"])
            stallThreshold = 0.08
            delta = 0.0001
            epsilon = 0.15
            T_smooth = 15
            ...
            for index, row in df.iterrows():
                ...
                mt = (hat_b[t_slope_1] - hat_b[t_slope_2]) / \
                    (t_slope_1 - t_slope_2)
                m.append(mt)
